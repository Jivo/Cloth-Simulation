...
namespace cgra {
	...
	void Mesh::setData_lines(const Matrix<double> &vertices,
		const Matrix<unsigned int> &lines) {
		line_mode = true;
		// Check to make sure that the number of columns in `vertices`
		// and `triangles` is correct
		if (vertices.numCols() != 3) {
		...
		for (unsigned int r = 0; r < vertices.numRows(); r++) {
			...
			Vertex v(
				glm::vec3(vert[0], vert[1], vert[2]),
				glm::normalize(glm::vec3(0, 0, 1)),// Normal is zero here
				default_color
			);
		...
		// Copy the data from `triangles` into `m_indices`
		for (unsigned int r = 0; r < lines.numRows(); r++) {
			const unsigned int *line = lines[r];

			// Add each index to `m_indices`
			m_indices.push_back(line[0]);
			m_indices.push_back(line[1]);
		}
	}

	void Mesh::updateVertices(std::vector<glm::vec3> positions) {
		for (int i = 0; i < m_vertices.size(); ++i) {
			m_vertices[i].m_position = positions[i];
		}
	...
	void Mesh::draw() {
		...
		GLenum drawMode = line_mode ? GL_LINES : GL_TRIANGLES;
