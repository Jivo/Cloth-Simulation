...
namespace cgra {
	...
	class Mesh {
		...
		glm::vec3 default_color = glm::vec3(1, 1, 0);
		struct Vertex {
			...
			Vertex(glm::vec3 pos, glm::vec3 norm) :
				m_position(pos), m_normal(norm), color(1, 1, 0) { }

			Vertex(glm::vec3 pos, glm::vec3 norm, glm::vec3 vColor) :
				m_position(pos), m_normal(norm), color(vColor) {}
		};
	...
	public:
		...
		void setData_lines(const Matrix<double> &vertices,
			const Matrix<unsigned int> &triangles);
		...
		void setDefaultColor(glm::vec3 defColor);
	};
